...
class SelectionDialog:
    def __init__(self, parent, store, path):
        ...
        w.geometry(f'{min(1020, w.winfo_screenwidth() - 40)}x{min(790, w.winfo_screenheight() - 100)}+10+10')
        w.minsize(850, 640)
        w.transient(parent)
        w.protocol('WM_DELETE_WINDOW', self.close)
        body = ttk.Frame(w, padding=16); body.pack(fill='both', expand=True)
        bottom = ttk.Frame(body); bottom.pack(side='bottom', fill='x')
        ...
        self.rows = ttk.Treeview(review, columns=('row', 'code', 'action'), show='headings', height=4, selectmode='browse')
        ...
        self.details = tk.Text(review, height=4, wrap='word', **text_colors())
        ...
        self.reviewed_button = ttk.Checkbutton(bottom, text='Выборка проверена инженером', variable=self.reviewed, command=self.update_commit)
        self.reviewed_button.pack(anchor='w', pady=(10, 6))
        footer = ttk.Frame(bottom); footer.pack(fill='x')
