...
@pytest.mark.parametrize("prefix", ["№", "ВО№", "ВО №"])
def test_service_letter_variants_removed(az, prefix):
    source = ("Комплектация по обосновывающему документу TEST.0001-АТТ.ОЛ1 "
              f"{prefix} АБВ-123 от 12.03.2024")
    result = az.anonymize(source)
    assert result["text"] == "Комплектация по обосновывающему документу TEST.0001-АТТ.ОЛ1"
    assert any(item.startswith("реквизиты письма:") for item in result["removed"])
...
@pytest.mark.parametrize("brand", [
    "Армтел", "ИКСИА", "Аэро Иксиа", "Теплолюкс", "Аналитика", "Эрис",
    "Феррум", "Телта", "Тропик", "PERCo", "Durcon", "TOFF", "НПО ФСА",
    "ООО НПО ФСА", "НЗСП", "ООО НЗСП",
])
def test_v17_confirmed_brand_residuals_removed(az, brand):
    source = f"Клапан {brand} IP66 УХЛ1 Ex d IIC T6 DN100 PN16 сталь 09Г2С"
    result = az.anonymize(source)
    assert brand.casefold() not in result["text"].casefold()
    assert all(token in result["text"] for token in
               ("IP66", "УХЛ1", "Ex d IIC T6", "DN100", "PN16", "09Г2С"))
