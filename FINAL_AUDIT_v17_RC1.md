...
- После Windows run #20 OCR TECH parser собирает полную Ex-маркировку из соседних glyph words одной строки и ячейки (`Ex d IIC T6`, `Ex db IIC T6 Gb`, `Ex ia IIC T4 Ga`, `1Ex d IIB T4`), включая типичные кириллические OCR-подмены. Бренд после выражения не входит в KEEP. При новом сбое тест печатает полный агрегированный список: ожидаемое production-правило, source OCR words, ближайший redaction, пересечения и pixel diagnostics.
- После Windows run #21 тем же cell/row parser собираются составные рейтинги и единицы (`PN1,6 МПа`, `PN16 bar`, раздельные `PN 25 МПа`, `DN 100`, напряжение и мощность). TECH preflight разделяет реальные дефекты (пересечение redaction либо физическая потеря тёмного core glyph) и информационные изменения рендеринга без потери glyph; CODE остаётся отдельным byte/pixel-strict invariant.
- После Windows run #22 Ex-классификатор получил локальную, не изменяющую PDF нормализацию OCR-confusables (`Е/Е→E`, `х/Х→X`, `И/І/l→I` в позиции gas group, `С→C`, `Т→T`, `б→6` только после temperature-prefix). KEEP по-прежнему строится из original glyph bbox всей последовательности; поддержаны несколько protection modes (`Ex eb mb IIC T4 Gb`) без захвата следующего бренда.
